...
- **Home screen** pressure module: dots, score, level, confidence, expected surge window, top-3 WHY, advice + detail, event impact (venue, distance from the trip, start / estimated end / surge window), source freshness and coverage moved to Information, reachable only from the top-right menu (#20). The primary button is the recommendation. Live next-bus cards keep PRT arrival times; all mock seat/walk claims were removed.
