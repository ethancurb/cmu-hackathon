...
- **Home screen** pressure module: dots, score, level, confidence and surge window; collapsible Advice with a short recommendation above sideways-scrolling square cards. The first card selects the quickest available bus itinerary for the current search and shows it on the map; subsequent event/weather/time/service cards expand for detail without contribution numbers. Timeline "What's happening" uses the same snapshots for the selected bar. Source freshness and coverage live on Information, reachable only from the top-right menu (#20). Live next-bus cards keep PRT arrival times; all mock seat/walk claims were removed.
