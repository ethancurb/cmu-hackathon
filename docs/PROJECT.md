...
Recommended stack remains a small Next.js/TypeScript frontend/API. Persistence is MongoDB Atlas with a time-series `stopEvents` collection ([ADR-0001](adr/0001-mongodb-for-mock-timeseries.md)); this supersedes the earlier Postgres note. Product framing: the primary surface is a commute-agent recommendation card, not a passive capacity display ([ADR-0002](adr/0002-commute-agent-as-differentiator.md)); the map, stops, and capacity chips remain, now as the canvas the plan draws on. Data-source verification still comes before provisioning extra infrastructure. The developer guide's `getpredictions` can supply specific vehicle identity, ETA, and current crowding together; if populated by PRT, it removes the need for phone tracking in the first milestone. Displaying an existing feed alone offers limited differentiation: prove the capacity-based improvement to the rider's existing journey. The proposed contract, access/quota limits, and first feasibility check are in [ARCHITECTURE](ARCHITECTURE.md). Domain vocabulary lives in [CONTEXT](../CONTEXT.md).
