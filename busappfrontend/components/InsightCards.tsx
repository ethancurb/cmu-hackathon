...
export function InsightCards({ cards, label }: { cards: InsightCard[]; label: string }) {
  ...
  return (
    <div className="min-w-0">
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className="text-footnote text-blue opacity-footnote">Swipe to explore · tap for more</span>
        {cards.length > 1 ? (
        ...
        {cards.map((card) => (
          <button
            ...
            className={cn("relative flex h-[168px] w-[168px] shrink-0 snap-start flex-col items-start border p-3 text-left text-blue", focusRing,
              card.recommended || expanded === card.id ? "border-ink-deep bg-canvas" : "border-border-soft bg-surface")}
          >
            <span className="flex w-full items-center gap-2">
              ...
              {card.recommended ? <span aria-hidden className="ml-auto h-2 w-2 shrink-0 bg-lime" /> : null}
            </span>
