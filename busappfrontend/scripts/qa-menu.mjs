...
try {
  ...
  if (await page.getByText('Sources', { exact: true }).count()) throw new Error('Sources still on home');
  if (await page.getByRole('link', { name: 'Information', exact: true }).count()) throw new Error('Information accessible outside menu');
  ...
  await page.getByRole('menuitem', { name: 'Information', exact: true }).click();
  ...
  console.log('menu -> Information -> Home ok');
  await page.getByRole('button', { name: 'Menu' }).click();
