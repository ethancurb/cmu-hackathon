...
const PITTSBURGH = { latMin: 40.25, latMax: 40.65, lngMin: -80.25, lngMax: -79.65 };
...
function readPoint(q: URLSearchParams, latKey: string, lngKey: string) {
  ...
  if (lat < PITTSBURGH.latMin || lat > PITTSBURGH.latMax || lng < PITTSBURGH.lngMin || lng > PITTSBURGH.lngMax) return undefined;
  return { lat, lng };
}
...
export async function GET(request: Request) {
  ...
  try {
    return Response.json(await livePressure({ location, destination, at, horizonMinutes: horizon }), { headers });
  } catch (error) {
